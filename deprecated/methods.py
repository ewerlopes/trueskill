def update(offence_gaussian, defence_gaussian, score, beta):
    """
    Updates the Poisson factor graph.
    :param offence_gaussian: the offence skill
    :param defence_gaussian: the defence skill
    :param score: the game score
    :param beta: the performance variance
    :return:
    """

    skills = [offence_gaussian, defence_gaussian]
...
if __name__ == '__main__':
    # TRAINING_PERCENTAGE_VAR = np.arange(0.1, 0.9, 0.1)
    # TEST_PERCENTAGE = 0.2
    # DATA_SELECTION = 1
    # NUMBER_OF_YEAR = 30
    #
    # matchOutcome = sio.loadmat('matchOutcomeHalo.mat')['matchOutcome']
    # print "Data dimension: {}".format(matchOutcome.shape)
    # numMatch, _ = matchOutcome.shape
    #
    # result_final_halo = dict([(key, []) for key in TRAINING_PERCENTAGE_VAR])
    # result_final_WLD_halo = dict([(key, []) for key in TRAINING_PERCENTAGE_VAR])
    # skill_percentage = dict([(key, []) for key in TRAINING_PERCENTAGE_VAR])
    # score_accuracy_percentage = dict([(key, []) for key in TRAINING_PERCENTAGE_VAR])
    # result_final_predict_win_halo = dict([(key, []) for key in TRAINING_PERCENTAGE_VAR])
    #
    # for trainingPercentage in TRAINING_PERCENTAGE_VAR:
    #     # decide which dataset to take
    #     start_time = timer()
    #     for idxYear in range(NUMBER_OF_YEAR):
    #         print('training Percentage = {}; idxYear = {}'.format(trainingPercentage, idxYear))
    #         # choose how many data points for testing
    #         # compute appropriate parameters
    #         tempInfoGain, tempWLD, tempSkill, tempScoreAccuracy, temppredictedWinResult = computeThreeMeasure_AllScoreDifference(
    #             matchOutcome, trainingPercentage, TEST_PERCENTAGE, 1)
    #         result_final_halo[trainingPercentage].append(tempInfoGain)
    #         result_final_WLD_halo[trainingPercentage].append(tempWLD)
    #         skill_percentage[trainingPercentage].append(tempSkill)
    #         score_accuracy_percentage[trainingPercentage].append(tempScoreAccuracy)
    #         result_final_predict_win_halo[trainingPercentage].append(temppredictedWinResult)
    #     end_time = timer()
    #
