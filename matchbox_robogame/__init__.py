...
from trueskill.factorgraph import (LikelihoodFactor, PriorFactor,
                                   SumFactor, Variable)
...
def setup(mu=MU, sigma=SIGMA, beta=BETA, tau=TAU,
    ...
    """
    if env is None:
        env = MatchBox(mu, sigma, beta, tau, draw_probability, backend)
    global_env.__matchbox__ = env
    return env
...
def global_env():
    ...
    try:
        global_env.__matchbox__
    except AttributeError:
        # setup the default environment
        setup()
    return global_env.__matchbox__
...
class MatchBox:
    ...
    def factor_graph_builders(self, rating_groups, ranks, weights, score):
        """Makes nodes for the MatchBox factor graph.
